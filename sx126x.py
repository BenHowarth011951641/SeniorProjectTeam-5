...
class sx126x:
    ...
    cfg_reg = [0xC2, 0x00, 0x09, 0x00, 0x00,
               0x00, 0x62, 0x00, 0x12, 0x43, 0x00, 0x00]
    get_reg = bytes(12)
    rssi = False
    addr = 65535
    serial_n = ""
...
# "20,868,Hello World"
    ...
    def receive(self):
        ...
        while (self.ser.inWaiting() > 0):
            ...
            # print the rssi
                ...
                # print('\x1b[3A',end='\r')
                #print(
                    #"the packet rssi value: -{0}dBm".format(256-r_buff[-1:][0]))
             #   self.get_channel_rssi()
            #else:
            #    pass
                # print('\x1b[2A',end='\r')
    ...
    def get_channel_rssi(self):
        ...
        if self.ser.inWaiting() > 0:
            ...
            #print(
                #"the current noise rssi value: -{0}dBm".format(256-re_temp[3]))
    ...
    def sendMessage(self, address, message):
        ...
        data = bytes([int(address) >> 8]) + bytes([int(address) & 0xff]) + bytes([self.offset_freq]) + \
            bytes([self.addr >> 8]) + bytes([self.addr & 0xff]) + \
            bytes([self.offset_freq]) + message.encode()
